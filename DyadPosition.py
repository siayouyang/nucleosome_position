...
##OptionParser
parser = OptionParser()
parser.add_option("-i", "--input",dest="input", help="centered input data in wiggle format")
...
parser.add_option("-F", "--filter", dest="filter", type="float", default=0.5, help="outputs only peaks with larger peak height. Default %default.")
...
def split_chr():
    wig_file = open(input, 'r')
    wig_file_readlines = wig_file.readlines()
    for b in range(0, len(chr)):
        chr_file = open(input + "_split_" + str(chr[b]) + ".temp", 'w')
...
def stddev(chr_a):
    ...
    for a in range(0, len(peaks_file_readlines)):
        # width=146bp
        peak_range = numpy.arange((int(peaks_file_readlines[a].split()[1])-73),(int(peaks_file_readlines[a].split()[1])+73))
        position_array = []
        frequency_array = []
        for b in peak_range:
            if b < 0:
        ...
        for c in range(0, len(position_array)):
            ...
            frequency = int(float(frequency_array[c]))
            n = 0
            while n < frequency:
        ...
        std_file.write(f'{peaks_file_readlines[a].split()[0]}\t{peaks_file_readlines[a].split()[1]}\t{peaks_file_readlines[a].split()[2]}\t{peaks_file_readlines[a].split()[3]}\t{peaks_std}\t{mean}\t{read_count}\n')
    peaks_file.close()
    wig_file.close()
    std_file.close()
...
def perform_exclusion(chr_a):
    ...
    for a in range(0, len(std_file_readlines)):
        ...
        list_val.append(float(std_file_readlines[a].split()[3]))   #peak_height
